...
const D3ChartPC = ({ category, sortOrder }) => {
  ...
  const createGraphs = (data, category) => {
    ...
    const x = d3
      .scaleLinear()
      .domain([0, d3.max(data, (d) => d[`${category}_number`])])
      .nice()
  ...
  //   // ウィンドウのサイズ変更イベントのリスナーを追加
  //   window.addEventListener("resize", handleResize);

  //   if (screenWidth >= 400 && screenWidth < 450) {
  //     // 600px を跨いだときに実行したい関数をここに記述
  //     console.log("Function to execute when screenWidth crosses 600px");
  //   }
